...
void App_DfPlayer_Init(App_Context *ctx)
{
    ...
    ctx->df_player_event_ctrl.prev_safe_state_for_audio = ctx->state.safe_state;
}
...
void App_DfPlayer_Task(App_Context *ctx)
{
    ...
    ActEcu_SafeState prev_safe_state = ctx->df_player_event_ctrl.prev_safe_state_for_audio;
    ActEcu_SafeState curr_safe_state = ctx->state.safe_state;
    ...
    /* 추가: FATAL -> NORMAL 복귀 시 이벤트 1번 음 1회 재생 */
    if ((prev_safe_state == ACTECU_SAFE_FATAL_NO_RESPONSE) &&
        (curr_safe_state == ACTECU_SAFE_NORMAL))
    {
        ctx->df_player_event_ctrl.pending_state = APP_AUDIO_STARTUP;
        ctx->df_player_event_ctrl.play_req = TRUE;
    }
